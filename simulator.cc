...
using namespace std;
void count_accesses(char*);
//string itos(int,long long unsigned);
void LeastRecentlyUsed(char*,string);

void count_accesses(char *file_name){
	string line;
	int number,count=0;
  	ifstream pf (file_name);
	map<int,long long unsigned> pa;
	map<int,long long unsigned> :: iterator pait;
  	if (pf.is_open()){
	...
	/*ofstream pcf;
	pcf.open("page-count.txt");*/

	for(pait=pa.begin();pait!=pa.end();pait++){
		count++;
		/*cout<<pait->first<<"    "<<pait->second<<endl;
		cout<<itos(pait->first,pait->second)<<endl;
		pcf<<itos(pait->first,pait->second)+"\n";*/
		
	}
	//pcf.close();
	cout<<"total number of distinct pages accessed are:  "<<count<<endl;
}

/*string itos(int first, long long unsigned second){
	stringstream ssfirst;
	stringstream sssecond;
	ssfirst<<first;
	string s1=ssfirst.str();
	sssecond<<second;
	string s2=sssecond.str();
	return s1+" -------------- "+s2;
}*/
